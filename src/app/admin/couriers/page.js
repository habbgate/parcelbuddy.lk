...
export default function AdminCouriers() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const d = await api(`/api/admin/couriers?page=${page}`);
  ...
  async function setStatus(id, status) {
    if (!confirm(`Set this courier to ${status}?`)) return;
    await api(`/api/admin/couriers/${id}/status`, { method: "PATCH", body: { status } });
    load();
  }

  return (
    <AdminShell>
      <h1 className="text-3xl font-extrabold text-navy">Couriers</h1>
      {loading ? (
          ...
          <table className="w-full text-left text-sm">
            <thead className="text-xs uppercase text-muted">
              <tr className="border-b border-border">
                <th className="py-2">Name</th><th>Status</th><th>ID</th><th>Deliveries</th><th>Rating</th><th>Earnings (Cash)</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {couriers.map((t) => (
                <tr key={t.id} className="border-b border-border last:border-0 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="mono">{formatLKR(t.totalEarningsLKR)}</td>
                  <td>
